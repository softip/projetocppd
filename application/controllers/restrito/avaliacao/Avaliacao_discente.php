...
class Avaliacao_discente extends CI_Controller {
    ...
    public function index() {
        if( in_array($this->crud->getState(),["list", "success","ajax_list", "read"]) ){
            ...
            $this->crud->fields("numero_processo", "data_abertura", "status","avaliacao_discente");                       
         }               
    ...
    public function _column_status($value, $row){
        ...
        if($status === "AVALIAÇÃO INICIADA"){
            if (!empty($processo->avaliacao_discente)){
                return "AVALIAÇÃO DA DISCENTE CONCLUÍDA";
            }else{
                return '<a href="'.site_url("restrito/avaliacao/avaliacao_discente/index/edit/$row->idavaliacao").'" class="btn btn-success">Inserir Avaliação Discente</a>';
            }            
        }
        return $status;
    }
