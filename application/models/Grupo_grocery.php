...
class Grupo_grocery extends Grocery_crud_model {
    ...
    function get_relation_array($field_name, $related_table, $related_field_title, $where_clause, $order_by, $limit = null, $search_like = null) {
        ...
        if ($field_name == 'pesquisador_idlider') {

            $relation_array = array();
            $this->db->select("idpesquisador, nome");
            $this->db->join("servidor", "servidor.idservidor = pesquisador.servidor_idservidor");
            $results= $this->db->get("pesquisador")->result();

            foreach ($results as $row) {
                $relation_array[$row->idpesquisador] = $row->nome;
            }

            return $relation_array;
        } else {
            return parent::get_relation_array($field_name, $related_table, $related_field_title, $where_clause, $order_by, $limit, $search_like);
        }
    }
